...
```{r}
bound2 <- bound2 %>% filter(TX_YR >= 2010 & COUNTRY == "US")
...
```{r}
#for gonzalo
summary(tableby( ~ Surv(PSURV_years, PCENS), data=bound2, times=1:10, surv.stats=c("NeventsSurv", "NriskSurv")))

summary(tableby( ~ Surv(PSURV_1year, PCENS_1year), data=bound2, times=1:1, surv.stats=c("NeventsSurv", "NriskSurv")))

summary(tableby( ~ Surv(PSURV_3year, PCENS_3year), data=bound2, times=1:3, surv.stats=c("NeventsSurv", "NriskSurv")))

summary(tableby( ~ Surv(PSURV_5year, PCENS_5year), data=bound2, times=1:5, surv.stats=c("NeventsSurv", "NriskSurv")))


```
**OS**
```{r}
fit1 <- survfit(Surv(PSURV_years, PCENS) ~ 1, data = bound2)
...
ggsurv1 <- ggsurvplot(
           ...
           palette = "npg",
           break.time.by = 1,  
           title = "Overall patient survival after liver transplantation for HCC in the US 5-year",
...
```{r}
t1 <- coxph(formula= Surv(PSURV, PCENS)~ 
              bound2$CIT+
              bound2$RSEX+
              bound2$RETHNIC+
              bound2$MELD+
...
```{r}
#for gonzalo
cox1 <- coxph(formula= Surv(PSURV_years,PCENS)~
              bound2$CIT+
              bound2$RSEX+
              bound2$RETHNIC+
              bound2$MELD+
