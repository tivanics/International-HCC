...
ukfull <- ukfull %>% mutate(RCREAT = case_when(
  RREN_SUP == "Pre-tx support" ~ 4,
  TRUE ~ RCREAT
))
...
**Isolate NASH pts**
```{r}
#Isolate NASH patients
ukfullNASH <- ukfull %>% filter(NASH==1)
```

**HCC**
```{r}
#Isolate HCC patients
#ukfull <- ukfull %>% filter(
#  hcc_wl== 2
#)
...
ukfull <- ukfull %>%
  mutate(MAX_TUMOR = case_when(
    MAX_TUMOUR1_SIZE_wl <= 3 ~ 1,
    MAX_TUMOUR2_SIZE_wl <= 3 ~ 1,
    MAX_TUMOUR3_SIZE_wl <= 3 ~ 1,
    MAX_TUMOUR4_SIZE_wl <= 3 ~ 1,
    MAX_TUMOUR5_SIZE_wl <= 3 ~ 1,
    MAX_TUMOUR1_SIZE_wl > 3 ~2,
    MAX_TUMOUR2_SIZE_wl > 3 ~2,
    MAX_TUMOUR3_SIZE_wl > 3 ~2,
    MAX_TUMOUR4_SIZE_wl > 3 ~2,
    MAX_TUMOUR5_SIZE_wl > 3 ~2
), na.rm= TRUE)

ukfull %>% group_by(MAX_TUMOR) %>% count() %>% ungroup()

ukfull <- ukfull %>%
  mutate(MAX_TUMOR = factor(MAX_TUMOR, labels = c("<3cm", ">3cm"))) 

#Total tumour diameter
which( colnames(ukfull)=="MAX_TUMOUR1_SIZE" ) #194
which( colnames(ukfull)=="MAX_TUMOUR5_SIZE" ) #198

ukfull$TOTAL_DIAMETER <- rowSums(ukfull[,c("MAX_TUMOUR1_SIZE_wl", "MAX_TUMOUR2_SIZE_wl", "MAX_TUMOUR3_SIZE_wl", "MAX_TUMOUR4_SIZE_wl", "MAX_TUMOUR5_SIZE_wl")], na.rm=TRUE)

ukfull$TOTAL_DIAMETER <- recode(ukfull$TOTAL_DIAMETER,
                                lo %thru% 5 ~ 1,
                                5.00001 %thru% hi ~ 2)
...
ukfull$TUMORNUM <- recode(ukfull$no_of_tumours_wl,
                          1~1,
                          2~2
                          )
    
#AFP
ukfull <- ukfull %>% mutate(AFP = AFP_LEVEL_wl)
                      
#AFP
ukfull$STRAT_AFP <- recode(ukfull$AFP_LEVEL_wl,
                           0 %thru% 20 ~ 1,
                           21 %thru% 400 ~ 2,
                           400.001 %thru% hi ~ 3
                           )

ukfull$STRAT_AFP <- factor(ukfull$STRAT_AFP, labels = c("AFP 0-20", "AFP 21-400", "AFP >400"))

#MILAN
ukfull <- ukfull %>% 
  mutate(EXTENDED = case_when(
    no_of_tumours_wl == 3 & MAX_TUMOUR1_SIZE_wl > 3 ~ 1,
    no_of_tumours_wl == 3 & MAX_TUMOUR2_SIZE_wl > 3 ~ 1,
    no_of_tumours_wl == 3 & MAX_TUMOUR3_SIZE_wl > 3 ~ 1,
    no_of_tumours_wl == 3 & MAX_TUMOUR4_SIZE_wl > 3 ~ 1,
    no_of_tumours_wl == 3 & MAX_TUMOUR5_SIZE_wl > 3 ~ 1,
    no_of_tumours_wl == 2 & MAX_TUMOUR1_SIZE_wl > 3 ~ 1,
    no_of_tumours_wl == 2 & MAX_TUMOUR2_SIZE_wl > 3 ~ 1,
    TRUE ~ 0
  ))

ukfull <- ukfull %>%
  mutate(MILAN = case_when(
    MAX_TUMOUR1_SIZE_wl > 5 | RADIO_EVIDENCE_VASC_INVASION_wl == 2 ~ 1,
    MAX_TUMOUR2_SIZE_wl > 5 | RADIO_EVIDENCE_VASC_INVASION_wl == 2 ~ 1,
    MAX_TUMOUR3_SIZE_wl > 5 | RADIO_EVIDENCE_VASC_INVASION_wl == 2 ~ 1,
    MAX_TUMOUR4_SIZE_wl > 5 | RADIO_EVIDENCE_VASC_INVASION_wl == 2 ~ 1,
    MAX_TUMOUR5_SIZE_wl > 5 | RADIO_EVIDENCE_VASC_INVASION_wl == 2 ~ 1,
    MAX_TUMOUR4_SIZE_wl >= 0.1 | MAX_TUMOUR5_SIZE_wl >= 0.1 ~ 1,
    no_of_tumours_wl >= 7 ~ 1,
    no_of_tumours_wl >= 4 | RADIO_EVIDENCE_VASC_INVASION_wl == 2 ~ 1,
    EXTENDED == 1 ~ 1,
    TRUE ~ 0
   ))

ukfull <- ukfull %>% mutate(
  MILAN = factor(MILAN, labels = c("Milan", "Extended"))
)

#Vascular invasion
ukfull$VASCULAR_INVASION <- recode(
  ukfull$RADIO_EVIDENCE_VASC_INVASION_wl,
                                   1~0,
                                   2~1,
                                   8~NA
                                   )
...
ukfull <- ukfull %>% mutate(
  VASCULAR_INVASION = factor(VASCULAR_INVASION, labels= c("No invasion", "Invasion")))

#Number of tumors
ukfull$TUMOR_NUMBER <- recode(ukfull$no_of_tumours_wl,
                               1 ~ 1,
                               2 %thru% hi ~ 2)
...
#AJCC - did i do this correctly? numbers make sense?
ukfull <- ukfull %>% mutate(
  AJCC = case_when(
    no_of_tumours_wl == 1 & MAX_TUMOUR1_SIZE_wl <= 2 ~ 1,
    no_of_tumours_wl == 1 & RADIO_EVIDENCE_VASC_INVASION_wl == 0 & MAX_TUMOUR1_SIZE_wl >= 2.1 ~ 2,
    no_of_tumours_wl == 1 & RADIO_EVIDENCE_VASC_INVASION_wl == 8 & MAX_TUMOUR1_SIZE_wl >= 2.1 ~ 2,
    no_of_tumours_wl == 2 & MAX_TUMOUR1_SIZE_wl == 2.4 ~ 2,
    no_of_tumours_wl == 1 & MAX_TUMOUR1_SIZE_wl >= 2.1 & RADIO_EVIDENCE_VASC_INVASION_wl == 1 ~ 3,
    MAX_TUMOUR2_SIZE_wl <= 5 ~ 3,
    MAX_TUMOUR3_SIZE_wl <= 5 ~ 3,
    MAX_TUMOUR4_SIZE_wl <= 5 ~ 3,
    MAX_TUMOUR5_SIZE_wl <= 5 ~ 3,
    no_of_tumours_wl > 1 & MAX_TUMOUR1_SIZE_wl > 5 ~ 4,
    no_of_tumours_wl > 1 & MAX_TUMOUR2_SIZE_wl > 5 ~ 4,
    no_of_tumours_wl > 1 & MAX_TUMOUR3_SIZE_wl > 5 ~ 4,
    no_of_tumours_wl > 1 & MAX_TUMOUR4_SIZE_wl > 5 ~ 4,
    no_of_tumours_wl > 1 & MAX_TUMOUR5_SIZE_wl > 5 ~ 4
  )
)

ukfull <- ukfull %>% mutate(
  AJCC = factor(AJCC, labels = c("T1A", "T1B", "T2")))
...
ukfullNASH <- ukfullNASH %>% mutate(
  COUNTRY = case_when(
    PCENS >= 0 ~ "UK"
  )
)

ukfull <- ukfull %>% rename(TRANSPLANT_UNIT = transplant_unit)

ukformerge <- ukfull %>% select(REGID, TX_YR, RAGE, GSURV, GCENS, PSURV, PCENS, LC, DAGE, DTYPE, DONCOD, DBMI, DSEX, DCMV, BLD_GP_MATCH, GRAFT_TYPE, CIT, RSEX, RETHNIC, BMI, WAITLIST_TIME, TRANSPLANT_UNIT, MELD, RREN_SUP, RVENT, RAB_SURGERY, RLIFE, RASCITES, RENCEPH, RBG, RANTI_HCV, RALBUMIN, RINR, RBILIRUBIN, RCREAT, COUNTRY, HCC_combined, UKT_PLDGRP, AJCC, MILAN, EXTENDED, TUMORNUM, TUMOR_NUMBER, AFP, STRAT_AFP, TOTAL_DIAMETER, MAX_TUMOR, COD_cardiovascular, COD_malignancy, COD_infection, COD_liverrelated, COD_other, NASH, ALF, HCV, PSC, HBV, PBC, ALD, AID, MET, OTH)

ukformerge2 <- ukfull %>% select(REGID, TX_YR, RAGE, GSURV, GCENS, PSURV, PCENS, LC, DAGE, DTYPE, DONCOD, DBMI, DSEX, DCMV, BLD_GP_MATCH, GRAFT_TYPE, CIT, RSEX, RETHNIC, BMI, WAITLIST_TIME, TRANSPLANT_UNIT, MELD, RREN_SUP, RVENT, RAB_SURGERY, RLIFE, RASCITES, RENCEPH, RBG, RANTI_HCV, RALBUMIN, RINR, RBILIRUBIN, RCREAT, COUNTRY, HCC_combined, UKT_PLDGRP, AJCC, MILAN, EXTENDED, TUMORNUM, TUMOR_NUMBER, AFP, STRAT_AFP, TOTAL_DIAMETER, MAX_TUMOR, COD_cardiovascular, COD_malignancy, COD_infection, COD_liverrelated, COD_other, NASH, ALF, HCV, PSC, HBV, HCC, PBC, ALD, AID, MET, OTH)

ukformergeNASH <- ukfullNASH %>% select(REGID, TX_YR, RAGE, GSURV, GCENS, PSURV, PCENS, LC, DAGE, DTYPE, DONCOD, DBMI, DSEX, DCMV, BLD_GP_MATCH, GRAFT_TYPE, CIT, RSEX, RETHNIC, BMI, WAITLIST_TIME, MELD, RREN_SUP, RVENT, RAB_SURGERY, RLIFE, RASCITES, RENCEPH, RBG, RANTI_HCV, RALBUMIN, RINR, RBILIRUBIN, RCREAT, COUNTRY, HCC_combined, UKT_PLDGRP, COD_cardiovascular, COD_malignancy, COD_infection, COD_liverrelated, COD_other)

#Feb 12
#ukformerge <- ukfull %>% select(REGID, TX_YR, RAGE, GSURV, GCENS, PSURV, PCENS, LC, DAGE, DTYPE, DONCOD, DBMI, DSEX, DCMV, BLD_GP_MATCH, GRAFT_TYPE, CIT, RSEX, RETHNIC, BMI, WAITLIST_TIME, TRANSPLANT_UNIT, MELD, RREN_SUP, RVENT, RAB_SURGERY, RLIFE, RASCITES, RENCEPH, RBG, RANTI_HCV, RALBUMIN, RINR, RBILIRUBIN, RCREAT, COUNTRY, HCC_combined, UKT_PLDGRP, COD_cardiovascular, COD_malignancy, COD_infection, COD_liverrelated, COD_other, NASH, ALF, HCV, PSC, HBV, PBC, ALD, AID, MET, OTH, HCC)
```
...
#Donor sex
corrdata$DSEX <- 
  recode(corrdata$SEX_CODE_donor,
                               c(1~1,
                                 0~0
                                 ))
...
corrdata <- corrdata %>% mutate(RCREAT = case_when(
  RREN_SUP == "Pre-tx support" ~ 4,
  TRUE ~ RCREAT
))
...
**Read in data**
```{r}
#uhndata <- read_csv2("/Users/Ivanics/Desktop/Research/92. UK vs US vs CAN/Data/UHN data/AI_file_merged_with_UHN_HCC_LT_masterfile_Jan3.csv", guess_max = 300000)

uhndata <- read_csv2("/Users/Ivanics/Desktop/Research/92. UK vs US vs CAN -- on git/Data/UHN data/UHN_HCC_LT_masterfile_merged_with_AI_file_Jan3.csv", guess_max = 300000)
...
```{r}
uhndata <- uhndata %>% 
  rename(REGID=subject_id)



#uhndata$TX_YEAR <- substring(uhndata$list1_delist_date, 1, 4)
uhndata$TX_YEAR <- substring(uhndata$Delisting_date, 1, 4)
uhndata$TX_YEAR <- as.numeric(uhndata$TX_YEAR)
...
#Keep only adult patients(alreadydone)
uhndata <- uhndata %>% 
  rename(RAGE = AGE)

#Remove missing survival information
uhndata$PTIME <- uhndata$Posttransplantsurvivaldays 
uhndata$PSTATUS <- uhndata$status
...
```{r}
#Donor age
uhndata <- uhndata %>% rename(
  DAGE = donor_age
)

#Donor type
uhndata <- uhndata %>% mutate(DCD = case_when(
  DCD == "1" ~ 1,
  DCD == "No" ~ 0,
  DCD == "0" ~ 0,
  DCD == "Yes" ~ 1,
...
uhndata <- uhndata %>% mutate(LDLT = case_when(
  Procedure == "LDLT" ~ 1,
  TRUE ~ 0
))
...
#BMI
uhndata <- uhndata %>% rename(DBMI = donor_bmi)

#Donor sex
uhndata$DSEX <- 
  recode(uhndata$donor_gender,
                               c("1"~1,
                                 "2"~0
                                 ))
...
#CIT
uhndata$CIT <- as.numeric(uhndata$CIT)
...
#uhndata <- uhndata %>% mutate(RAGE = RAGE)
...
#Waiting list
#uhndata$DAYSWAIT_CHRON <- difftime(uhndata$list1_delist_date, uhndata$list1_date, units = c("days"))
uhndata$DAYSWAIT_CHRON <- difftime(uhndata$Delisting_date, uhndata$list1_date, units = c("days"))
...
#Renal support
uhndata$RREN_SUP <- 
  recode(uhndata$pretx1_dialysis,
                               c(1~1,
                                 0~0
                               ))
...
#Previous abdominal surgery
uhndata$RAB_SURGERY <- recode(uhndata$pretx1_surg,
                               c(1 ~ 1,
                                 0 ~ 0,
                                 TRUE ~ NA_real_))
...
uhndata <- uhndata %>% mutate(
  RENCEPH = case_when(
    pretx1_he == 0 ~ 0,
    pretx1_he == 1 | pretx1_he == 2 ~ 1,
    pretx1_he == 9 ~ NA_real_
  )
) %>%
  mutate(RENCEPH = factor(RENCEPH, labels = c("Not encephalopathic", "Encephalopathic")))
...
uhndata <- uhndata %>% mutate(RCREAT = case_when(
  RREN_SUP == "Pre-tx support" ~ 4,
  TRUE ~ RCREAT
))
...
corrdataformerge2 <- corrdata %>% select(REGID, TX_YR, RAGE, GSURV, GCENS, PSURV, PCENS, DAGE, DTYPE, DONCOD, DBMI, DCMV, DSEX, GRAFT_TYPE, CIT, RSEX, RETHNIC, BMI, WAITLIST_TIME, MELD, RREN_SUP, RBG, RANTI_HCV, RINR, RBILIRUBIN, RCREAT, COUNTRY, HCC_combined, UKT_PLDGRP, NASH, COD_cardiovascular, COD_malignancy, COD_infection, COD_liverrelated, COD_other, HCC, UHN, NASH, ALF, HCV, PSC, HBV, PBC, ALD, AID, MET, OTH)
...
#Feb 12
#corrdataformerge <- corrdata %>% select(REGID, TX_YR, RAGE, GSURV, GCENS, PSURV, PCENS, DAGE, DTYPE, DONCOD, DBMI, DCMV, DSEX, GRAFT_TYPE, CIT, RSEX, RETHNIC, BMI, WAITLIST_TIME, MELD, RREN_SUP, RBG, RANTI_HCV, RINR, RBILIRUBIN, RCREAT, COUNTRY, HCC_combined, UKT_PLDGRP, NASH, COD_cardiovascular, COD_malignancy, COD_infection, COD_liverrelated, COD_other, UHN, NASH, ALF, HCV, PSC, HBV, PBC, ALD, AID, MET, OTH, HCC)
```
...
#Feb 12
unosformerge2 <- unosdata %>% select(REGID, TX_YR, RAGE, GSURV, PSURV, PCENS, GCENS, DAGE, DTYPE, DONCOD, DBMI, DCMV, DSEX, BLD_GP_MATCH, GRAFT_TYPE, CIT, RSEX, RETHNIC, BMI, WAITLIST_TIME, TRANSPLANT_UNIT, MELD, RREN_SUP, RVENT, RAB_SURGERY, RLIFE, RASCITES, RENCEPH, RBG, RANTI_HCV, RALBUMIN, RINR, RBILIRUBIN, RCREAT, COUNTRY, HCC_combined, UKT_PLDGRP, NASH, COD_cardiovascular, COD_malignancy, COD_infection, COD_liverrelated, COD_other, NASH, ALF, HCV, PSC, HBV, PBC, ALD, AID, MET, OTH, HCC)
...
bound5hccwithall <- bind_rows(unosformerge2, ukformerge2, corrdataformerge2)
```
...
write_csv(bound5hccwithall, path = "/Users/Ivanics/Desktop/GitHub/International-HCC/Data/bound5hccwithall.csv")
#write_csv(boundnash, path = "/Users/Ivanics/Desktop/boundnash.csv")
```
